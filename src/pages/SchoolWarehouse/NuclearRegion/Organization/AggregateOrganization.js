import React from 'react';
import DataAggregationMechanism from '../Components/DataAggregationMechanism';
import { SCHOOLS_WAREHOUSE_TYPE } from 'src/shared/constants/DataFixed';
import { ROUTER_NAME } from 'src/pages/routeConfig';

// import PropTypes from 'prop-types';

AggregateOrganization.propTypes = {};

function AggregateOrganization() {
  return (
    <DataAggregationMechanism
      category={SCHOOLS_WAREHOUSE_TYPE.ORGANIZATION}
      pageName={ROUTER_NAME.DATA_AGGREGATION_MECHANISM_ORGANIZE_TRAINING}
    />
  );
}

export default AggregateOrganization;
