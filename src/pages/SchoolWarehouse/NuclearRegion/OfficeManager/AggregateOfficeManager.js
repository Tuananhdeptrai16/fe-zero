import React from 'react';
import DataAggregationMechanism from '../Components/DataAggregationMechanism';
import { SCHOOLS_WAREHOUSE_TYPE } from 'src/shared/constants/DataFixed';
import { ROUTER_NAME } from 'src/pages/routeConfig';

// import PropTypes from 'prop-types';

AggregateOfficeManager.propTypes = {};

function AggregateOfficeManager() {
  return (
    <DataAggregationMechanism
      category={SCHOOLS_WAREHOUSE_TYPE.OFFICE}
      pageName={ROUTER_NAME.DATA_AGGREGATION_MECHANISM_OFFICE_MANAGER}
    />
  );
}

export default AggregateOfficeManager;
